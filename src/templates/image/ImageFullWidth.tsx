...
export function ImageFullWidth({ section }: { section: Section }) {
  ...
  return (
    ...
    >
      <div className={`group w-full overflow-hidden ${effectClass}`}>
        {errored ? (
          <div className="w-full h-[400px]"><ImageFallback /></div>
        ) : (
          <img
            src={imageUrl}
            alt={caption || 'Full width image'}
            loading="lazy"
            className={`w-full h-[400px] object-cover transition-transform duration-200 ease-out hover:scale-105${isClickEnlarge ? ' cursor-pointer' : ''}`}
