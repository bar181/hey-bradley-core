...
/* --------------------------------------------------------------------- */
/*  BlogFeaturedGrid — First article featured, rest in 2-col grid         */
/* --------------------------------------------------------------------- */
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        {/* Featured article */}
        <article
          className={`group relative rounded-2xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-xl opacity-0 animate-card-reveal ${effectClass}`}
        >
            ...
            {showTags && featured.tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-3">
                {featured.tags.map((tag) => (
                  <span key={tag} className="text-xs px-2 py-0.5 rounded-full bg-white/20">{tag}</span>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {rest.map((article, idx) => (
              <article
                ...
                style={{ animationDelay: `${(idx + 1) * 100}ms`, background: section.style.background }}
              >
