...
export function NavbarSticky({ section }: { section: Section }) {
  ...
  return (
    <nav
      className={`sticky top-0 z-50 backdrop-blur-md border-b border-[var(--hb-border)]/30 bg-[var(--hb-bg)]/80 transition-shadow duration-200 ${scrolled ? 'shadow-md' : 'shadow-none'}`}
      style={{
        background: section.style.background ? `${section.style.background}cc` : undefined,
