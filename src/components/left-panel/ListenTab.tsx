      ...
      <div className="px-4 pt-2 pb-1 flex flex-col items-center gap-2">
        ...
        {activeProfile && (
          <div ref={popoverRef} className="relative inline-flex">
            <button type="button" data-testid="listen-active-personality-chip" data-personality-id={personalityId ?? undefined} aria-haspopup="menu" aria-expanded={popoverOpen} aria-label={`Change personality (active: ${activeProfile.label})`} onClick={() => setPopoverOpen((v) => !v)} title={`Change personality (active: ${activeProfile.label})`} className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider bg-white/10 text-white/80 border border-white/20 hover:bg-white/20 transition-colors focus-visible:ring-2 focus-visible:ring-[var(--hb-accent)] focus-visible:outline-none"><span>{activeProfile.emoji ? `${activeProfile.emoji} ` : ''}{activeProfile.label.split(' ')[0]}</span><span aria-hidden className="text-[8px] leading-none">▾</span></button>
            {popoverOpen && (
