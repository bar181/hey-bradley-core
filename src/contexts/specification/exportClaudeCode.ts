...
function buildClaudeMd(phase: PhaseCard): string {
  // P111 / A1 — preamble leads with DDD bounded contexts + cited ADR IDs so
  // the consuming developer or AI tool sees the architectural layer first.
  const ctxLines: string[] =
    phase.dddOutput && phase.dddOutput.contexts.length > 0
      ? phase.dddOutput.contexts.map((c) => `- **${c.name}** — ${c.responsibility}`)
      : ['> Bounded contexts: not extracted for this phase. Use /ddd-map to generate.']
  const adrRows: string[] =
    phase.adrRefs.length > 0
      ? phase.adrRefs.map((a) => `| ${a.id} | ${a.title} |`)
      : ['> No ADRs cited yet. Use /adr-new to draft architectural decisions.']
  return [
    ...
    ...ctxLines,
    '',
    '> See `ddd-contexts.md` for full prose + relationships + anti-corruption layers.',
    '',
    ...
    ...(phase.adrRefs.length > 0
      ...
      : adrRows),
    '',
    '> See `adr-bundle/<id>.md` for full ADR text per cited reference.',
    '',
    '## AISP atoms',
    '',
    'PATCH · INTENT · SELECTION · CONTENT · ASSUMPTIONS · DECOMP · PROCESS · DDD · AGENT',
    '',
    '> See `aisp/phase-aisp.md` for verbatim Σ/Γ/Λ/Ε blocks.',
    '',
    '## Implementation plan',
    '',
    '> See `implementation-plan.md` for phase/sprint/agent breakdown.',
    '> See `tdd-scaffold.md` for Given/When/Then per phase.',
    '',
    '## Methodology (7-step)',
    '',
...
export function buildClaudeCodeBundle(
  ...
  // P111 / A1 — Re-order so DDD + ADR appear FIRST after CLAUDE.md preamble.
  // Architectural layer leads (bounded contexts → cited ADRs → AISP math →
  // enriched plan → TDD scaffold), then process map + human spec + agents.
  // Existing back-compat files (`human-spec/implementation-plan.md` and
  // `adrs/<id>.md` stubs from P96) RETAINED at end of array.
  files.push({ path: 'CLAUDE.md', content: buildClaudeMd(phase) })

  // P111 / A1 — PRIORITY 1: DDD bounded contexts as readable prose.
  files.push({ path: 'ddd-contexts.md', content: buildDddContexts(phase) })

  // P111 / A1 — PRIORITY 2: per-ADR full text via `readAdr` callback (ADR-138).
  ...
  // P111 / A1 — PRIORITY 3: AISP Σ blocks (the math).
  files.push({ path: 'aisp/phase-aisp.md', content: buildAispSpec(phase) })

  // P111 / A1 — PRIORITY 4: enriched implementation plan (PROCESS_ATOM prose).
  files.push({
    path: 'implementation-plan.md',
    content: buildImplementationPlanEnriched(phase),
  })

  // P111 / A1 — PRIORITY 5: TDD Given/When/Then scaffold (ADR-128).
  const tddScaffold = buildTDDScaffold(phase)
  files.push({ path: 'tdd-scaffold.md', content: tddScaffold.markdown })

  // Process map + human-spec narratives follow the architectural priority block.
  ...
  // P96 back-compat — `adrs/<id>.md` stub references remain at end of array.
  for (const adr of phase.adrRefs) {
