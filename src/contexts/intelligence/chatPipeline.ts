...
// Decision record: docs/adr/ADR-046-stt-web-speech.md
//
 ...
 */
async function runLLMPipeline(text: string, source: 'chat' | 'listen' | 'test'): Promise<{
  applied: number
  summary: string
}> {
  const adapter = useIntelligenceStore.getState().adapter
  if (!adapter) return { applied: 0, summary: '' }
  const configState = useConfigStore.getState()
  const systemPrompt = buildSystemPrompt({ configJson: configState.config })
...
export async function submit(opts: ChatPipelineOptions): Promise<ChatPipelineResult> {
  ...
  try {
    const llm = await runLLMPipeline(text, opts.source)
    if (llm.applied > 0) {
      return {
        ok: true,
