...
- **Current Phase:** P64 / OC-3 SEALED — Templates Round 1 (Whiteboard mode). +3 vertical templates (coffee-roaster e-commerce, dev-conference event, podcast-show audio); registry 23 → 26. 14/14 new tests GREEN. **439/439 cumulative PURE-UNIT GREEN.** P63 / OC-2 sealed at `ac6f973` (3-Card Onboarding Stub + ADR-088 Mode Architecture + ADR-089 Agentics Data Model). P62 / OC-1 sealed at `6a86d5c` (Visual Polish Floor, 405 GREEN). **NEXT: P65 / OC-2.5 INSERTED** — owner-redirected scope: Design Token System + Component Quality Standard BEFORE more template work (foundational design-system per ADR-087). Subsequent: OC-4 templates Round 2 builds on the new token system. Open planning phases: P61 (18-sprint OC roadmap, `64e305c`/`b1a9652`) + P61b (10-sprint AW roadmap, `8025878`). **ADR numbering reassignment:** ADR-087 → Design Token System (was: Mobile UX); ADR-090 → reserved for OC-5 Mobile UX (pending owner UX-spec). Sprints G/H/I-remainder/original-J-Agentic-Support deferred to Tier-2.
...
- **ADRs:** 88 Accepted on disk; ADR-087 + ADR-090 reserved. Range ADR-045 through ADR-089 + reserved-090. Recent additions: ADR-082 (Open Core RC, P58), ADR-083 (Test Library Architecture / prompt corpus, P59), ADR-084 (Comprehensive QA Architecture, P60), ADR-085 (Multi-Page MVP, P61), ADR-086 (Process Pages content/runtime split, P61), ADR-088 (Mode Architecture, P63 / OC-2), ADR-089 (Agentics Data Model, P63 / OC-2). **ADR-087 reserved → Design Token System + Component Quality Standard (P65 / OC-2.5; reassigned 2026-04-30).** **ADR-090 reserved → Mobile UX redesign (OC-5; pending owner UX-spec; was ADR-087 before reassignment).** Numbering has 11 documented gaps (002-004, 006-009, 034-037) plus 3 stub-then-superseded duplicates (ADR-051/052/053 each have a P21 stub + a later Accepted file under the same number; see `docs/adr/README.md`).
...
- **Tests:** Cumulative **439/439 PURE-UNIT GREEN at P64 / OC-3 seal** (392 P60 baseline + 3 P60.5 AISP-trace-auto-expand + 10 P62 OC-1 design-token spec + 20 P63 OC-2 mode selector spec + 14 P64 OC-3 templates Round 1). P59 baseline = 298 Sprint N at `c00c2b7` + ~17 P58 RC + ~51 P59 prompt-library per ADR-083. P60 added 26 specs (9 flagship + 6 LLM matrix + 6 personality + 5 templates). Full corpus is 846+ tests across 73+ spec files (includes legacy + skipped suites); 439 is the curated PURE-UNIT seal-gate cumulative-regression subset.
