...
- **Current Phase:** P67b / Close the Gap SEALED — 4-agent dispatch (A1 ChatInput orchestrator carry-forward THIRD-attempt with line-explicit instructions; A2 sub-page hero polish; A3 mobile audit; A4 ADR-094 + tests + EOP). A1: ChatInput.tsx 1013 → 850 LOC (-163, -16%) consuming the 3 P67-shipped sub-components; ≤700 target missed (further extraction of ChatThread + useCallback bodies needed → P67c). A2: 4/5 sub-pages improved (AISP 7→9, OpenCore 8→9, Research 6→9, Progress 7→9; Blog already 8). A3: 5 mobile surfaces audited; 2 fixed (ListenModeDemo + ChatModeDemo flex-wrap + responsive padding); 3 already clean. A4: ADR-094 Professional Grade Standard Accepted (per-surface ≥8, library mean ≥8.0, touched-surface mean ≥8.5). 1 inline test fix at seal (P66.9 popover-state check now spans ChatInput + ChatInputPersonalityPopover after A1 moved state with the component). **604/604 cumulative PURE-UNIT GREEN** (was 570). Visual polish 7.9 → 8.3 library mean; touched-surface 8.7 EXCEEDS the 8.5 per-touched-surface gate. Predecessors: P67 Polish Wave 2 (`17c9635`); P66 Polish Wave 1 (`34699d4`); P66/OC-MKTG (`62af4a4`); P65b (`e7b6af2`); P65 (`261d840`); P64 (`0701b37`); P63 (`ac6f973`); P62 (`6a86d5c`). Open planning: P61 (`64e305c`/`b1a9652`) + P61b (`8025878`). ADR ledger: ADR-087 Design Tokens; ADR-088 Mode Architecture; ADR-089 Agentics Data Model; ADR-091 Canonical Component Quality; ADR-092 Polish Sprint Architecture; ADR-093 Component Decomposition Standard; **ADR-094 Professional Grade Standard (P67b)**; ADR-090 reserved for OC-5 Mobile UX. Carry-forward: ChatThread extraction (P67c — pushes orchestrator to ~600 LOC); Polish Wave 3 (legacy untouched surfaces); per-mode UI variants (AW work); ≤300 LOC ChatInput target (deferred). NEXT: Polish Wave 3 (legacy sweep) OR P67c (ChatThread extraction) OR OC-4 (templates) — owner choice.
...
- **ADRs:** 93 Accepted on disk; ADR-090 reserved. Range ADR-045 through ADR-094 with reserved-090. Recent additions: ADR-082 (Open Core RC, P58), ADR-083 (Test Library Architecture, P59), ADR-084 (Comprehensive QA Architecture, P60), ADR-085 (Multi-Page MVP, P61), ADR-086 (Process Pages content/runtime split, P61), **ADR-087 (Design Token System, P65 / OC-2.5)**, ADR-088 (Mode Architecture, P63 / OC-2), ADR-089 (Agentics Data Model, P63 / OC-2), **ADR-091 (Canonical Component Quality, P65b / OC-2.5 Wave 2)**, **ADR-092 (Polish Sprint Architecture, P66 Wave 1)**, **ADR-093 (Component Decomposition Standard, P67 / Polish Wave 2)**, **ADR-094 (Professional Grade Standard, P67b / Close the Gap)**. **ADR-090 reserved → Mobile UX redesign (OC-5; pending owner UX-spec).** Numbering has 11 documented gaps (002-004, 006-009, 034-037) plus 3 stub-then-superseded duplicates (ADR-051/052/053 each have a P21 stub + a later Accepted file under the same number; see `docs/adr/README.md`).
...
- **Tests:** Cumulative **604/604 PURE-UNIT GREEN at P67b / Close the Gap seal** (392 P60 baseline + 3 P60.5 + 10 P62 OC-1 + 20 P63 OC-2 + 14 P64 OC-3 + 11 P65 OC-2.5 + 31 P65b OC-2.5 Wave 2 + 47 P66 Polish Sprint Wave 1 + 42 P67 Polish Wave 2 + 34 P67b Close the Gap). P59 baseline = 298 Sprint N at `c00c2b7` + ~17 P58 RC + ~51 P59 prompt-library per ADR-083. P60 added 26 specs. Full corpus is 846+ tests across 73+ spec files (includes legacy + skipped suites); 604 is the curated PURE-UNIT seal-gate cumulative-regression subset.
