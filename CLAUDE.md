...
- **Current Phase:** P65b / OC-2.5 Wave 2 SEALED — Canonical Component Quality Standard. 7 canonical section components refactored to consume ADR-087 token contract (4 Hero variants + 2 Feature variants + 1 Testimonial). ADR-091 Accepted (Canonical Component Quality Standard). Hero variants implement scroll-reveal via IntersectionObserver (browser-native, no library). Feature + Testimonial cards have hover-lift + elevated-shadow Tailwind transitions. Existing 26 templates inherit improvements automatically. 31/31 new tests GREEN. **481/481 cumulative PURE-UNIT GREEN.** Predecessors: P65 / OC-2.5 sealed at `261d840` (Design Token System); P64 / OC-3 (`0701b37`); P63 / OC-2 (`ac6f973`); P62 / OC-1 (`6a86d5c`). Open planning phases: P61 (`64e305c`/`b1a9652`) + P61b (`8025878`). **ADR numbering:** ADR-087 → Design Token System (P65); ADR-088 → Mode Architecture (P63); ADR-089 → Agentics Data Model (P63); ADR-091 → Canonical Component Quality (P65b); ADR-090 → reserved for OC-5 Mobile UX (pending owner UX-spec). NEXT: OC-4 Templates Round 2 (healthcare + non-profit + search/filter; new templates inherit canonical components from day one). Sprints G/H/I-remainder/original-J-Agentic-Support deferred to Tier-2.
