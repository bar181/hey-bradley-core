...
| **P27** | Sprint C P2 — LLM-Native AISP (capstone-thesis demonstration) | **90/100** (Grandma 76 / Framer 88 / Capstone 96) — **plateau broken (+2 vs P26 89)** | 10/10 DoD; brutal-review composite 90 ≥ greenlight gate | (this session) | A1 NEW `src/lib/schemas/intent.ts` Zod (mirrors Σ exactly: 6-verb enum + 21 target-type enum + 1-based-index + confidence range) + `aisp/llmClassifier.ts` `llmClassifyIntent(text)` sends INTENT_ATOM verbatim as system prompt; cost-cap pre-check skips when sessionUsd ≥ capUsd × 0.9; safeParseJson strips markdown fences; falls through gracefully on parse fail / over-cap / network error. A2 NEW `src/components/shell/AISPTranslationPanel.tsx` collapsible "How I understood this" UI: input/verb/target/params/confidence/source-pill (Rules emerald / LLM indigo / Fallthrough amber)/rationale; `data-testid="aisp-translation-panel"`. A3 NEW `docs/adr/ADR-056-llm-native-aisp.md` thesis claim (full Accepted). BONUS NEW `docs/adr/ADR-055-aisp-conversion-verification.md` (AISP authoring rubric + 30-line worked-example of P15-P26 trajectory in proper AISP format Ω/Σ/Γ/Λ/Ε with `≜`/`≡`/`∎`). chatPipeline runs LLM-AISP only when rule-based < threshold. NEW `tests/p27-llm-aisp.spec.ts` (9 PURE-UNIT cases; first-pass GREEN). Build green at 558 KB gzip (+0.2 KB). Brutal-review (4 reviewers, P23-P27 cumulative): R1 Grandma 76 (above raised 75 bar) / R2 Functionality 88 / R3 Security 92 / R4 Architecture 87 (-1 carryforward debt penalty: C04+C15+C16+C17 still outstanding). **Composite 90 → Sprint D greenlight conditional on P28 closing must-fix carryforwards.** Three first-class AISP artifacts in repo (atom + classifier + UI) + 30-line AISP-format trajectory doc. $0 spent in dev. |
