...
| **P34** | Sprint E P1 — UI Closure + Assumptions Engine + brutal-honest review fix-pass | **95/100** estimated post-fix (Grandma 79 / Framer 89 / Capstone 98) | 9/9 DoD + 6/6 must-fix closed | (pending commit) | **Wave 1 (Sprint D UI closure):** A1 wired `chatPipeline.ts` ChatPipelineResult with `aisp + templateId`; AISPTranslationPanel renders inline under each bradley reply. A2 NEW `TemplateBrowsePicker.tsx` (~110 LOC) — visual grid by category; `/browse` slash command. **Wave 2 (Assumptions + Clarification):** A3 NEW `aisp/assumptions.ts` + `assumptionStore.ts` (kv ring buffer; zero migration). A4 NEW `ClarificationPanel.tsx` — 3 button + escape; ChatInput re-runs pipeline w/ rephrasing. A5 ADR-063 full Accepted. **Brutal-honest review (4 reviewers PASS):** R1 UX 86 (4 must-fix); R2 Func 84 (1 must-fix); R3 Sec 82 (2 LOW); R4 Arch 84 (1 must-fix). **Fix-pass closed all 6 must-fix + 2 LOW**: R1 F1 persistent /browse link after first message; R1 F2 "yours" tag dead branch removed; R1 F3 Escape-key dismiss on both panels; R1 F4 mutually-exclusive panels; R1 L1+L2 copy alignment ("X% match"); R1 L5 aria-live region; R2 F1 clarification gated on `!result.ok`; R2 L1 cue-vs-intent precedence; R2 L4 VERB_CUES.hide disambiguated from .remove; R3 F2 BYOK regex pre-write filter + 1024-char clamp in assumptionStore; R3 F5 8192-char clamp in generateAssumptions (DoS); R4 F1 SECTION_CUES typed via `Partial<Record<AllowedSectionType, ...>>`. NEW `tests/p34-wave1-ui-closure.spec.ts` (17) + `tests/p34-assumptions.spec.ts` (28; 25/35 prompt coverage gate met) + `tests/p34-fix-pass.spec.ts` (21). **Final regression: 157/157 GREEN.** Build green. **Sprint E greenlight CONFIRMED.** |
