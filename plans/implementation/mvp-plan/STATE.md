...
| **P34** | Sprint E P1 — UI Closure + Assumptions Engine | **94/100** estimated (Grandma 77 / Framer 92 / Capstone 98) | 9/9 DoD | (pending commit) | **Wave 1 (Sprint D UI closure):** A1 wired `chatPipeline.ts` ChatPipelineResult with `aisp + templateId`; ChatInput captures via `pendingAispRef` and renders `AISPTranslationPanel` inline under each bradley reply (closes R1 F2). A2 NEW `TemplateBrowsePicker.tsx` (~110 LOC) — visual grid by category (theme/section/content); cards show name + kind chip + first example + "yours" tag; click fills input; `/browse` slash command + empty-state hint discoverability (closes R1 F3+F4). **Wave 2 (Assumptions + Clarification):** A3 NEW `aisp/assumptions.ts` (~140 LOC) — `generateAssumptions({text, intent})` returns up to 3 ranked candidates via section/verb cue tables; `shouldRequestAssumptions(intent)` triggers on null/<0.7/null target; `ASSUMPTIONS_TRIGGER_THRESHOLD=0.7`. A3 NEW `aisp/assumptionStore.ts` — kv-backed ring buffer (last 50 records); zero migration. A4 NEW `ClarificationPanel.tsx` (~75 LOC) — 3 option buttons + "something else" escape; confidence percentage chips; rationale; ChatInput re-runs pipeline with `rephrasing` on accept + persists via `recordAcceptedAssumption`. A5 ADR-063 full Accepted. NEW `tests/p34-wave1-ui-closure.spec.ts` (17) + `tests/p34-assumptions.spec.ts` (28; **25/35 prompt coverage gate met**). **Cumulative regression: 136/136 GREEN** (P29-P34 + 3 fix-passes). Build green. **Sean-from-Good-Will-Hunting clarification UX in production.** |
