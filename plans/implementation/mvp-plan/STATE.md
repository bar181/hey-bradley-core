...
| **P33** | Sprint D P5 — Content + Template Bridge (SPRINT D CLOSE) | **93/100** (+1; Grandma 76 / Framer 91 / Capstone 98) — Sprint D close climb | 8/8 DoD | (pending commit) | Template type extended w/ optional `kind` / `category` / `examples` (ADR-058 evolution path realized). NEW `generate-headline` template registered (kind:'generator'; first generator). Library decoration prefers template metadata over BASELINE_META (legacy 3-template shim). `twoStepPipeline.ts` kind-dispatch: `if (tpl.kind === 'generator')` → `generateContent({ text, sectionType })` → `heroHeadingPath` resolver → JSON-Patch envelope; `TwoStepResult.generated?` exposed for UI. Patcher path unchanged (P28 backward-compat). ADR-062 full Accepted. NEW `tests/p33-content-bridge.spec.ts` — **11 PURE-UNIT cases GREEN first-pass**. P29 test relaxed (`≥0` generators) for forward-compat. **Sprint C-D regression P28-P33 = 60/60 GREEN.** Build green. ~30m / ~3× velocity. $0. **SPRINT D COMPLETE: 5 phases / 5 ADRs (058-062) / 54 PURE-UNIT cases / 1 migration / $0. 4-atom AISP architecture (INTENT+SELECTION+CONTENT+PATCH) in production.** |
