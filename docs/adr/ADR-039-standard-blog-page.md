...
1. **`hero` (id `hero-01`)** — heading, subheading, single CTA pointing to the article anchor.
2. **`article` (id `article-01`)** — title, hero image, author byline, plain-text/markdown body of three to four paragraphs.
...
### Negative
- The `article` section type must accept a long-form body (`bodyMarkdown` or equivalent). If the renderer for `article` is currently a stub, Phase 15 must finish it. This is a prerequisite, not a regression.
...
### Risks
- If the `article` section type evolves a richer schema (cover variants, author cards, table of contents) the example must be re-validated. Mitigated by the `_meta.schema` discipline in ADR-031.

---

## Related ADRs

- ADR-022: Section Type Registry — establishes `hero`, `article`, `footer` as registered types
